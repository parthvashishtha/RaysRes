...
function getSchemaVersionString(schema, api) {
  if (!_chunkWCDZQK62cjs.isOAS31.call(void 0, api)) {
...
function toJSONSchema(data, opts = {}) {
  ...
  const schemaAdditionalProperties = _chunkWCDZQK62cjs.isSchema.call(void 0, schema) ? schema.additionalProperties : null;
  const {
  ...
  if (_chunkWCDZQK62cjs.isRef.call(void 0, schema)) {
    refLogger(schema.$ref, "ref");
    return transformer({
      $ref: schema.$ref
    });
  }
  if (_chunkWCDZQK62cjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
  ...
  if (_chunkWCDZQK62cjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
    ...
    if (hasSchemaType(schema, "array")) {
      if ("items" in schema) {
        if (!Array.isArray(schema.items) && Object.keys(schema.items).length === 1 && _chunkWCDZQK62cjs.isRef.call(void 0, schema.items)) {
  ...
  if (_chunkWCDZQK62cjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && globalDefaults && Object.keys(globalDefaults).length > 0 && currentLocation) {
    try {
  ...
  if (_chunkWCDZQK62cjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && "enum" in schema && Array.isArray(schema.enum)) {
...
function getParametersAsJSONSchema(operation, api, opts) {
  ...
  const typeKeys = _chunkXWUEF5DOcjs.getExtension.call(void 0, _chunkXWUEF5DOcjs.PARAMETER_ORDERING, api, operation).map((k) => k.toLowerCase());
...
//# sourceMappingURL=chunk-VJPJCQC3.cjs.map